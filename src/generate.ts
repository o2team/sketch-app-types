...
const extraSymbols = [
  '_MSArtboardGroup', '_MSAssetCollection', '_MSBaseGrid',
...
function generateDo (context: SketchContext, symbolMatch: RegExp, outputFile: string) {
  ...
  // Mocha globalSymbolNames() get errors on Sketch 53+
  const bridgeController = MOBridgeSupportController.sharedController()
  const symbols = NSMutableArray.new()

  // Exported objects
  symbols.addObjectsFromArray(_exportedObjects.allKeys())

  // ObjC runtime
  // TODO: to fixed errors
  ...
  // BridgeSupport
  const bridgeSupportSymbols = bridgeController.performQueryForSymbolsOfType([MOBridgeSupportFunction.class(), MOBridgeSupportConstant.class(), MOBridgeSupportEnum.class()])
  ...
  const symbolNames = uniqueArray(array(symbols)
    .map(s => String(s))
...
function array (objects: NSArray) {
